...
(defn gather-deps
  ...
  (doseq [{:keys [provides]} require-foreign-deps]
    (let [n (symbol (first provides))]

      (planck-require n false)
      (prn :foreign-dep-cache? (get-cache cljsbuild-out n))
      ))
  ...
  (let [sources (reduce (fn [m namespace]
        ...
        goog (reduce (fn [m file]
                       (let [path (munge file)]
                         (-> (assoc m path (resource file))
                             (update "preload_js" (fnil conj []) path)))) {} (reverse require-goog))

        foreign-deps (reduce (fn [m {:keys [file]}]
                               (let [path (munge file)]
                                 (-> (assoc m path (resource file))
                                     (update "preload_js" (fnil conj []) path)))) {} (reverse require-foreign-deps))
    ...
    (merge-with into sources foreign-deps caches macros goog)))
...
(let [deps-path (user-path (or (get-named-arg "deps") "live-deps.clj"))
      ...
      {:keys [error] :as deps} (-> (line-seq *in*)
                                   (string/join)
                                   (r/read-string))
      _ (when error (println :error error))
      deps (->> (:value deps)
